def reward_function(params):
    ...
    maxReward = 1e2
    minReward = 1e-3
    directionThresholdD = 10.0
    absSteeringThreshold = 30

    #car variables
    onTrack = params['all_wheels_on_track']
    ...
    def onTrackReward(current_reward, onTrack):
        if not onTrack:
            current_reward = minReward
        else:
            current_reward = maxReward
    ...
    def distance_from_center_reward(current_reward, track_width, distance_from_center):
        ...
        if distance_from_center <= marker_1:
            ...
            current_reward = minReward  # likely crashed/ close to off track
    ...
    def direction_reward(current_reward, waypoints, closest_waypoints, heading):
        ...
        if direction_diff > directionThresholdD:
            current_reward *= 0.5
    ...
    def steering_reward(current_reward, steering):
        ...
        if abs(steering) > absSteeringThreshold:
            current_reward += 0.8
        return current_reward
